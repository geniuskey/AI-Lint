...
import { locate } from '../src/content/anchor-locator.js'
...
/** happy-dom에는 XPath가 없다. 실제 Chrome에서 쓰는 './tag[n]' 형태만 흉내 낸다. */
...
describe('locate', () => {
  ...
  it('Confluence 앵커가 아니면 null이다', () => {
    const other: SourceAnchor = { kind: 'pdf', page: 1, bbox: [0, 0, 1, 1] }
    expect(locate(other, document)).toBeNull()
  })
  ...
  it('xpath가 어긋나면 인용문으로 찾는다', () => {
    const found = locate(anchor({ xpath: './p[99]' }), document)
    expect(found?.textContent).toBe('결제 승인 흐름을 정리한다')
  })

  it('망가진 xpath여도 죽지 않고 인용문으로 넘어간다', () => {
    const found = locate(anchor({ xpath: './p[[[' }), document)
    expect(found?.textContent).toBe('결제 승인 흐름을 정리한다')
  })
  ...
  it('어디에도 없으면 null이다', () => {
    const found = locate(anchor({ xpath: '', textQuote: { exact: '없는 문장' } }), document)
  ...
  it('본문 컨테이너가 없으면 body에서 찾는다', () => {
    document.body.innerHTML = '<article><p>컨테이너 밖 문장</p></article>'
    const found = locate(anchor({ xpath: '', textQuote: { exact: '컨테이너 밖 문장' } }), document)
